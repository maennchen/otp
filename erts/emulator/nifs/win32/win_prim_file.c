...
posix_errno_t efile_open_at(efile_data_t *dir, const efile_path_t *path,
    ...
    /* Win32 cannot open a name relative to a directory handle, so this uses
     * the native call that takes the directory in its object attributes. */
    ...
    options = EFILE_FILE_SYNCHRONOUS_IO_NONALERT;
    ...
    status = NtCreateFile(&handle, access_flags, &object_attributes,
...
posix_errno_t efile_set_handle_permissions(efile_data_t *d, Uint32 permissions) {
    ...
    if(!GetFileInformationByHandleEx(w->handle, FileBasicInfo, &info, sizeof(info))) {
        return windows_to_posix_errno(GetLastError());
    }
    ...
    if(!SetFileInformationByHandle(w->handle, FileBasicInfo, &info, sizeof(info))) {
        return windows_to_posix_errno(GetLastError());
    }
...
posix_errno_t efile_set_handle_time(efile_data_t *d, Sint64 a_time, Sint64 m_time,
    ...
    if(!SetFileTime(w->handle, &created, &accessed, &modified)) {
        return windows_to_posix_errno(GetLastError());
    }
