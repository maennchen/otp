 ...
 * sentinel atom (am_big / am_badarg / am_no_integer / am_not_a_list /
 * am_system_limit). For the list source a successful parse returns a {Int,Rest}
...
static Eterm c2int_parse(Process *p, Eterm *bif_args)
{
    ...
    if (c2int_bits_per_digit_pow2(base)) {
        ...
        if (BIG_NEED_SIZE(wlen) - 1 > BIG_ARITY_MAX) {
            res = am_system_limit;
        } else {
            Uint need = BIG_NEED_SIZE(wlen) + (want_tuple ? 3 : 0);
            Eterm *hp_start = HAlloc(p, need);
            Eterm *hp_end = hp_start + need;
            Eterm *hp2 = hp_start;

            MOVE_DIGITS(BIG_V(hp2), pow2_w, wlen);
            res = big_norm(hp2, wlen, (short) sign);
            if (is_nil(res)) {
                res = am_system_limit;
                HRelease(p, hp_end, hp_start);
            } else {
                hp2 = is_big(res) ? hp2 + BIG_NEED_SIZE(wlen) : hp_start;
                if (want_tuple) {
                    Eterm tpl = TUPLE2(hp2, res, rest_out);
                    hp2 += 3;
                    res = tpl;
                }
                HRelease(p, hp_end, hp2);
            }
        }
    } else {
    ...
    if (n_limbs <= C2INT_HORNER_MAX_LIMBS) {
        ...
        if (BIG_NEED_SIZE(acc_sz) - 1 > BIG_ARITY_MAX) {
            res = am_system_limit;
        } else {
            Uint need = BIG_NEED_SIZE(acc_sz) + (want_tuple ? 3 : 0);
            Eterm *hp_start = HAlloc(p, need);
            Eterm *hp_end = hp_start + need;
            Eterm *hp2 = hp_start;

            MOVE_DIGITS(BIG_V(hp2), acc, acc_sz);
            res = big_norm(hp2, acc_sz, (short) sign);
            if (is_nil(res)) {
                res = am_system_limit;
                HRelease(p, hp_end, hp_start);
            } else {
                hp2 = is_big(res) ? hp2 + BIG_NEED_SIZE(acc_sz) : hp_start;
                if (want_tuple) {
                    Eterm tpl = TUPLE2(hp2, res, rest_out);
                    hp2 += 3;
                    res = tpl;
    ...
    /*
     * Build the result on the process heap exactly once. big_norm() either
     * writes a BIG_NEED_SIZE(final_sz)-word bignum and returns it, or collapses
     * to an immediate small (consuming no heap). Over-allocate the maximum we
     * might need (bignum + an optional {Int,Rest} tuple for the list source),
     * then HRelease the words big_norm/the tuple did not actually use, so no
     * uninitialized heap words are left live (which would trip the heap check).
     */
    if (BIG_NEED_SIZE(final_sz) - 1 > BIG_ARITY_MAX) {
        res = am_system_limit;
    } else {
        Uint need = BIG_NEED_SIZE(final_sz) + (want_tuple ? 3 : 0);
        Eterm *hp_start = HAlloc(p, need);
        Eterm *hp_end = hp_start + need;

        hp = hp_start;
        MOVE_DIGITS(BIG_V(hp), src + src_off[0], final_sz);
        res = big_norm(hp, final_sz, (short) sign);
        if (is_nil(res)) {
            res = am_system_limit;
            HRelease(p, hp_end, hp_start);
        } else {
            /* Advance past the bignum only if one was actually built. */
            hp = is_big(res) ? hp + BIG_NEED_SIZE(final_sz) : hp_start;
            if (want_tuple) {
