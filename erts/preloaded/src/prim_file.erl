...
%% Opening a name against an open directory does not resolve a path, so the
...
open({#file_descriptor{module = ?MODULE} = Dir, Name}, Modes) ->
    try
        ...
        case open_at_nif(DirRef, encode_path(Name), Modes) of
            {ok, Ref} -> {ok, make_fd(Modes, Ref)};
            {error, Reason} -> {error, Reason}
        end
    catch
...
write_file_info_1(Filename, Info, TimeType) ->
    try
        write_file_info_2({path, encode_path(Filename)}, Info, TimeType)
    catch
        throw:Reason -> {error, Reason};
        error:_ -> {error, badarg}
    end.
